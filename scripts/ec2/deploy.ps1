...
Invoke-Ssh "chmod +x ~/start-backend.sh ~/deploy-frontend.sh && sed -i 's/\r$//' ~/start-backend.sh ~/deploy-frontend.sh"
...
if (-not $BackendOnly) {
    if (-not $SkipBuild) {
        ...
        $env:VITE_API_URL = "http://${Ec2Host}:8080"
...
Write-Host "  API:     http://${Ec2Host}:8080"
