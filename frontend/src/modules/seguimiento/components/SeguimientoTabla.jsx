      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
        <thead>
          <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
            {['Estudiante', 'Empresa', 'Docente', 'Corte', 'Estado', 'Última Actividad', 'Acciones'].map((h) => (
        ...
        <tbody>
          {practicas.map((p) => {
            const badge = BADGE[p.estadoSeguimiento] || { label: p.estadoSeguimiento, bg: '#f3f4f6', color: '#374151' };
            
            // formatear fecha 
            const fechaVal = p.fechaUltimaActividad ? new Date(p.fechaUltimaActividad) : null;
            const fechaStr = fechaVal ? fechaVal.toLocaleDateString() : '—';
            
            return (
              <tr key={p.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                <td style={{ padding: '12px 14px' }}>
                  <div style={{ fontWeight: 600, color: '#111827' }}>{p.estudiante}</div>
                </td>
                <td style={{ padding: '12px 14px' }}>
                  <div style={{ fontWeight: 600, color: '#374151' }}>{p.empresa}</div>
                </td>
                <td style={{ padding: '12px 14px', color: '#374151' }}>{p.docente}</td>
                <td style={{ padding: '12px 14px', color: '#374151', textAlign: 'center' }}>
                  {p.corte}
                </td>
                <td style={{ padding: '12px 14px' }}>
